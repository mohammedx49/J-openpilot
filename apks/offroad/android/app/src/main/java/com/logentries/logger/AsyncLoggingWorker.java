...
public class AsyncLoggingWorker {
    ...
    public AsyncLoggingWorker(Context context, boolean useSsl, boolean useDataHub, String logToken,
        ...
        String tokenId = logToken.substring(0, 8);

        queue = new ArrayBlockingQueue<String>(QUEUE_SIZE);
        localStorage = new LogStorage(context, tokenId);
        appender = new SocketAppender(useSsl, useDataHub, dataHubAddress, dataHubPort, logToken, logHostName);
        appender.start();
        started = true;
    }

    public AsyncLoggingWorker(Context context, boolean useSsl, String logToken) throws IOException {
        this(context, useSsl, false, logToken, null, 0, true);
    }

    public AsyncLoggingWorker(Context context, boolean useSsl, String logToken, String dataHubAddr, int dataHubPort)
            throws IOException {
        this(context, useSsl, true, logToken, dataHubAddr, dataHubPort, true);
    }

    public synchronized void addLineToQueue(String line) {
    ...
    private class SocketAppender extends Thread {
        ...
        public SocketAppender(boolean useSsl, boolean isUsingDataHub, String dataHubAddr, int dataHubPort,
        ...
        private void openConnection() throws IOException, InstantiationException {
            if(leClient == null){
                leClient = new LogentriesClient(useSsl, isUsingDataHub, dataHubAddr, dataHubPort, token);
            }
